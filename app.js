...
function createGameCard(game) {
    ...
    const statusColor = game.status === 'Complete' ? 'text-neon-green' : 
                        game.status === 'Beta' ? 'text-yellow-400' : 'text-gray-400';

    return `
        <article class="group border border-retro-gray rounded-lg overflow-hidden hover:border-neon-green transition-all neon-glow bg-retro-dark">
            <div class="p-5">
                <div class="flex items-start justify-between mb-3">
                    <div>
                        ...
                        <h3 class="font-medium text-white text-lg leading-tight group-hover:text-neon-green transition-colors">
                            ${game.name}
                        </h3>
                    </div>
                    <span class="text-xs ${statusColor} font-medium border border-current px-2 py-1 rounded">
                        ${game.status}
                    </span>
                </div>
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    ${featureTags.map(tag => `
                        <span class="text-xs bg-neon-green/10 text-neon-green px-2 py-1 rounded">
                            ${tag}
                        </span>
                    `).join('')}
                </div>
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    ${platformBadges.map(p => `
                        <span class="text-xs bg-retro-gray text-gray-400 px-2 py-1 rounded">
                            ${p}
                        </span>
                    `).join('')}
                </div>
                ...
                <div class="flex items-center gap-3 pt-4 border-t border-retro-gray">
                    <a href="${game.githubUrl}" 
                       ...
                       class="flex items-center gap-2 text-sm text-white hover:text-neon-green transition-colors">
                    ...
                    <a href="game.html?id=${game.id}" 
                       class="ml-auto text-sm text-neon-green hover:underline">
                        Details →
                    </a>
                </div>
            </div>
